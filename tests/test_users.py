...
@pytest.fixture
def get_all_users():
    return requests.get(f'{main_route()}/users').json()
...
def get_user_by_id():
    ...
    return requests.get(f'{main_route()}/users/{id}').json()
...
def get_user_by_handle():
    ...
    return requests.get(f'{main_route()}/users/{handle}').json()
...
def update_user_attribute():
    id = dummy_user_info().get('id')
    updated_user = requests.put(f'{main_route()}/users/{id}/handle/some_test').json()
    corrected_user = requests.put(f'{main_route()}/users/{id}/handle/fakeuser').json()
...
def delete_user():
    id = dummy_user_info().get('id')
    return requests.delete(f'{main_route()}/users/{id}').json()
